...
async function seedRoles(permissionByCode: Map<string, PermissionRecord>, resetMode: boolean) {
  ...
  for (const roleDefinition of roleDefinitions) {
    ...
    for (const menuAccess of roleDefinition.menuAccesses) {
      await prisma.roleMenuAccess.upsert({
        ...
        update: {},
        create: { roleId: role.id, topMenu: menuAccess.topMenu, viewKey: menuAccess.viewKey }
      });
    }
  }
...
async function seedContentCatalog() {
  ...
  for (const templateEntry of seedTemplates) {
    const savedTemplate = await prisma.template.upsert({
      ...
      update: { ...templateEntry, isActive: true },
      create: { ...templateEntry, isActive: true }
    });
  ...
  for (const element of seedElements) {
    const existing = await prisma.element.findFirst({ where: { name: element.name } });

    if (existing) {
      await prisma.element.update({ where: { id: existing.id }, data: element });
      continue;
    }
...
async function seedSections(
  ...
  for (const [slug, visits] of visitSeeds.entries()) {
    ...
    if (section) {
      await prisma.section.update({ where: { id: section.id }, data: { visits } });
    }
  }
  ...
  const unrestrictedRoles = ["Administrador", "Desenvolvedor"].map((roleName) => roleMap.get(roleName)).filter(Boolean);
  const publishingRoles = ["Publicador Geral", "Editor"].map((roleName) => roleMap.get(roleName)).filter(Boolean);

  for (const role of [...unrestrictedRoles, ...publishingRoles]) {
    for (const section of sectionMap.values()) {
      await prisma.roleSectionAccess.upsert({
        where: { roleId_sectionId: { roleId: role!.id, sectionId: section.id } },
        update: {},
        create: { roleId: role!.id, sectionId: section.id }
      });
    }
  }
  ...
  for (const role of unrestrictedRoles) {
    for (const contentType of contentTypeMap.values()) {
      await prisma.roleContentTypeAccess.upsert({
        where: { roleId_contentTypeId: { roleId: role!.id, contentTypeId: contentType.id } },
        update: {},
        create: { roleId: role!.id, contentTypeId: contentType.id }
      });
    }
  }

  for (const role of publishingRoles) {
...
async function seedSystemRecords(roleMap: Map<string, RoleRecord>) {
  for (const systemEmail of systemEmails) {
    const existing = await prisma.systemEmail.findFirst({ where: { name: systemEmail.name } });

    if (existing) {
      await prisma.systemEmail.update({ where: { id: existing.id }, data: systemEmail });
      continue;
    }

    await prisma.systemEmail.create({ data: systemEmail });
  }

  const adminEmail = process.env.ADMIN_EMAIL ?? "admin@abbatech.local";
  const seedUsers = createSeedUsers(adminEmail, process.env.ADMIN_PASSWORD ?? "Refresh123!").filter(
    (seedUser) => seedUser.email !== adminEmail
  );
...
async function seedNewsletter() {
  ...
  for (const recipient of newsletterRecipients) {
    ...
    await prisma.newsletterRecipient.upsert({
      ...
      update: { name: recipient.name, groupId: group.id, consentAt: new Date() },
      create: { email: recipient.email, name: recipient.name, groupId: group.id, consentAt: new Date() }
    });
  }
  ...
  if (existingCampaign) {
    await prisma.newsletterCampaign.update({
      ...
      data: { ...launchCampaign, recipientGroupId: leadGroup.id }
    });
    return;
  }
...
async function seedContents(
  ...
  for (const content of contents) {
    ...
    if (!section || !contentType) {
      throw new Error(`Dados demo incompletos para o conteudo ${content.slug}`);
    }

    const canonicalUrl = `${process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3100"}/${content.slug}`;
    const existing = await prisma.content.findUnique({ where: { slug: content.slug }, include: { seo: true } });
    ...
    await prisma.contentRevision.create({
      data: {
        ...
        snapshot: JSON.parse(JSON.stringify(savedContent))
      }
    });
  }
}
...
async function main() {
  ...
  await seedSystemRecords(roleMap);
  await seedNewsletter();

  const template = templateMap.get("pagina-padrao");

  if (!template) {
    throw new Error("Template demo pagina-padrao nao encontrado.");
  }

  await seedContents(admin.id, sectionMap, contentTypeMap, template.id);
  await seedPrivacyRequests();
  process.stdout.write(`Seed demo concluido. Admin: ${bootstrap.adminEmail}\n`);
}
